...
const AXES: [string, string][] = [
  ...
  ["Structured", "Flexible"],
  ["Quiet", "Outgoing"],
  ["Even-keeled", "Expressive"],
  ["Accommodating", "Direct"],
  ["Serious", "Playful"],
  ["Improvised", "Meticulous"],
  ["Cautious", "Adventurous"],
  ["Pragmatic", "Sensitive"],
  ["Trusting", "Careful"],
  ["Practical", "Imaginative"],
  ["Plain-spoken", "Diplomatic"],
  ["Confident", "Humble"],
...
const MODELS: Record<string, [string, string][]> = {
  ...
  xai: [
    ["gateway", "xai/grok-4-mini"],
    ["gateway", "xai/grok-4"],
    ["gateway", "xai/grok-4"],
  ],
  mistral: [
    ["gateway", "mistral/mistral-small"],
...
function composeTone(vals: number[], axes: [string, string][]): string {
  const traits = axes
    .map((ax, i) => ({
      word: (vals[i] < 50 ? ax[0] : ax[1]).toLowerCase(),
      d: Math.abs(vals[i] - 50),
    }))
...
function buildScript(a: Answers): string {
  ...
  const env =
    a.provider && a.provider.startsWith("local")
      ? "OLLAMA_KEY"
      : `${(a.provider || "cf").split(" ")[0].toUpperCase()}_API_KEY`;
  return [
    `cat > ${name}.json <<'JSON'`,
    `{ "binding": "${name}",`,
    `  "persona": ${JSON.stringify(persona)},`,
  ...
  return (
    <div class="wiz">
      <div class="term__bar">
        <span></span>
        <span></span>
        <span></span> bullmoose · connect an agent
      </div>
      <div class="wiz__scroll">
        ...
        {STEPS.slice(0, step).map((k) => (
          <div class="wiz__hist">
            ...
            <p class="wiz__a">
              <span>›</span> {String((answers as any)[k])}
            </p>
          </div>
        ))}
        ...
        {!done && (
          <div class="wiz__now">
            <p class="wiz__q">
              {typed}
              {!ready && <span class="t-cursor" />}
            </p>
            {ready && (
              <div class="wiz__in">
                {key === "location" && (
                  <div class="wiz__opts">
                    <button onClick={() => commit("location", "cloud")}>
                      ☁︎ cloud <em>always-on</em>
                    </button>
                    <button onClick={() => commit("location", "desktop")}>
                      ▤ my-desktop <em>homelab</em>
                    </button>
                  </div>
                )}
                {key === "iq" && (
                  <div class="wiz__opts wiz__scale">
                    {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
                      <button
                        onClick={() => commit("iq", n)}
                        title={n <= 3 ? "quick & cheap" : n <= 7 ? "balanced" : "deep & pricey"}
                      >
                        {n}
                      </button>
                    ))}
                ...
                {key === "name" && (
                  <form
                    onSubmit={(e: Event) => {
                      e.preventDefault();
                      if (text.trim()) commit("name", text.trim());
                    }}
                  >
                    ...
                    <input
                      autoFocus
                      value={text}
                      placeholder="e.g. maya, scout, ledger…"
                ...
                {key === "purpose" && (
                  <div>
                    ...
                    <form
                      onSubmit={(e: Event) => {
                        e.preventDefault();
                        if (text.trim()) commit("purpose", text.trim());
                      }}
                    >
                      <span class="wiz__prompt">$</span>
                      <input
                        autoFocus
                        value={text}
                        placeholder="click a card, or type it…"
                        onInput={(e: any) => setText(e.target.value)}
                      />
                      <button type="submit" class="wiz__go">
                        →
                      </button>
                    </form>
                  </div>
                    ...
                    {showSliders ? (
                      ...
                      >
                        ...
                        <input
                          autoFocus
                          value={text}
                          placeholder="cheeky, to the point, encouraging…"
                          onInput={(e: any) => setText(e.target.value)}
                        />
                        <button type="button" class="wiz__go" onClick={() => setShowSliders(true)}>
                          /personas
                        </button>
                      </form>
                    )}
        ...
        {done && (
          <div class="wiz__out">
            <p class="wiz__q">
              Done. Paste this into your terminal to connect <b>{answers.name}</b>:
            </p>
            <pre class="wiz__cmd">
              <code>{command}</code>
            </pre>
            <div class="wiz__outbtns">
              <button class="wiz__go" onClick={() => navigator.clipboard?.writeText(command)}>
                Copy command
              </button>
            ...
            <p class="wiz__note">
              ...
              <a href={`https://github.com/ericdmoore/bullmoose.cc/blob/main/docs/DEPLOY.md`}>
