...
async function applyProposal(
  ...
  switch (row.kind) {
    ...
    case "grant-request": {
      ...
      // The chokepoint write. authorization.proposalId is what T2 stamps as
      // via_proposal_id on the membership-log row; the provenance trio
      // additionally records last_writer_invocation = the proposal id (== the
      // invocation id the proposal projects over, via provCtx above).
      await store.insertContactCard(access.accountId, cardRow, { proposalId: row.id });
