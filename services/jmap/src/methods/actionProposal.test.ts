...
describe("approving a recipient grant-request APPLIES the contact write", () => {
  ...
  it("a HUMAN approve inserts the contact into the target book with the proposal as its why", async () => {
    ...
    // s10 T2 contract: when book_membership_log lands (T2 merges before this),
    // the applied write's log row must carry the authorizing proposal:
    // const log = h.w.db.query<{ via_proposal_id: string }>(
    //   "SELECT via_proposal_id FROM book_membership_log WHERE account_id = ? AND book_id = ?",
    //   ACCOUNT, "ab_gov",
    // )[0]!;
    // expect(log.via_proposal_id).toBe("inv_w");
