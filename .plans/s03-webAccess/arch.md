...
> readme asks *what should exist*, this says *what we build, on what, and why that
> shape*.
>
...
> *named-principal* sharing (`shareWith`), not "send someone a link." See §3.4.
...
webmail be a competent plain client *and* a multiplayer one without contorting either.
...
- **Blob GC hazard.** The draft is explicit: *"A blob referenced by a FileNode MUST NOT
  be expired or garbage collected while the FileNode exists."* Upload blobs today are
  transient-by-intent; a referenced blob becomes **pinned**. This is the one
  non-obvious storage change and it must land *with* the schema, not after.
...
rule — *attachment ≥ N bytes → create a FileNode under `role:"home"`/Attachments,
keep the link in the message* — and Files is populated on day one instead of being an
empty drive.
...
`ActionProposal` is a *read model* over invocations plus a proposal payload, exposed as
...
asking for a permission and an agent proposing a reply are the same interaction — *what,
why, approve/deny* — and unifying them means one review surface, not two
...
forensic view is one query rather than a reconstruction. The audit log answers *who
did*; the grants answer *who could*; this answers *who actually touched this record*.
...
thing: a per-`kind` (optionally per-subject) rule that says *auto-approve within these
bounds*. s03 **records and applies** decisions and surfaces the "want this automatic?"
...
`assignee` + `claimedAt` on the thread projection so webmail can render *"Allen is
drafting"*. No new mechanism — just surfacing one.
...
**The agent console is the boundary object.** s03 builds the *screens* (per-agent
...
- **Bulk = server-side.** Query-filter-batch over 5k items cannot round-trip; it needs
  an `urn:bullmoose:agent` method that applies an action to a *query*, with a
