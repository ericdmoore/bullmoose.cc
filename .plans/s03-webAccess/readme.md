...
> both productive. This is the _design conversation_, not a build plan — `arch.md` and
...
>
> _What design affordances would be needed for both to be productive?_
...
| **Power-user (you)** | **CLI**, also                | `bullmoose admin …`; and per `mcp-auth.md` §9 the CLI is deliberately the _human_ path for raw secrets — plaintext must not detour through a web tier |

**The CLI is dual-audience**, not agent-only. Worth naming so its ergonomics don't get
under-invested.

**Webmail is human-_first_, but not agent-_naive_.** Agents write into the human's
...
surfaces use new parts — per the `serverless-jmap.md` §19 boundary: _boolean → keyword;
structured → vendor capability, visible only to clients that speak it._ The approval
...
> _"Stalwart is the server: it stores mail, speaks SMTP, runs spam, holds auth.
> **Bulwark is the JMAP client** you point your browser at."_
...
**Verdict: don't port or fork.** Take its _realm decomposition_ (mail + calendar +
...
- **Front** — internal comments rendered _inline in the thread timeline_; **shared
...
- **Intercom Fin** — the **handoff protocol** (AI answers, escalates _with context_).
...
**side-stepping attachment size limits.** Big file → Files realm → send a _link_, not
...
- **Inbound symmetry is nearly free.** Attachments already land in R2 as blobs
  (`Mailstore.putBlob` **[live]**), so _"promote attachment ≥ N to a Files node, keep a
  link in the message"_ makes Files useful on day one instead of an empty drive.
- **Link-sharing already ships.** ~~Sending a file link outside bullmoose pulls forward
  ...
  Phase-6 ACL epic actually gates is _named-principal_ sharing (the draft's
...
3. **Thread view** — ownership, collision ("Allen is drafting"), agent reasoning,
   context sidebar.
4. **Agent console** — bindings, MCP credential _references_, A2A grants, audit
   (both views, §6).
...
### Promote repetition to policy — _the through-line_
...
| ----------------------------- | ----------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| **1. Reversible**             | move, label, classify, create contact, file organize, event _without_ invites | **real undo** — local state                                                 |
| **2. Retractable pre-commit** | send reply, start thread, event _with_ invites                                | **not undo — a hold window.** Gmail's "undo send" is a delay buffer         |
| **3. Irreversible**           | anything that left the building; data an agent already read                   | remediation only — revoking a grant stops _future_ access, it can't un-read |
...
- **right but not now** → a snooze; must _not_ count against the agent
...
Selecting _by query_ has real consequences: **server-side execution** (5,000 items
...
| Question                                  | View             | Mode                            |
| ----------------------------------------- | ---------------- | ------------------------------- |
| _"Can Allen even do that?"_               | **per-agent**    | authorization — forward-looking |
| _"Who could have messed up VendorsBook?"_ | **per-resource** | forensic — backward-looking     |

The forensic view is itself two queries: **who _could_** (the authorization set) and
**who _did_** (`grant_audit`). Show them together — the gap is the finding. A wide
...
`delete`. Render what it _allows_. Likewise surface dangerous **combinations** —
...
2. **Provenance must be cross-realm.** `grant_audit` only fires on _delegated_ access
   ...
   calendar, and files need the equivalent _"who/what last wrote this."_

---

## 8. Dependencies & boundaries

- **s04 (AgentOS) owns the governance model.** Its readme already lists _Gatekeeper ·
  Budget Constraints · ACLs (People Accessing Agents, Agents Accessing Tools/Data)_ —
...
- **`mcp-auth.md` §9** already specifies the credential admin plane — reuse it rather
  ...
  are the WebUI's _best_ case (nothing sensitive typed); raw API keys are the one flow
...
2. **Files: conform to a JMAP draft, or design a vendor capability?** Blocked on the
   §4 research item.
3. **Where does the approval queue's _collection_ live** — a new JMAP collection under
...
- Public file sharing (the multi-principal ACL epic) — Files _for me_ first.
