...
| **T3** — attachment sidestep | ✅ **done** (2026-08-13 inbound · 2026-08-19 outbound) | Inbound attachments over the threshold (default 5 MiB, non-inline) mint FileNodes in an `Attachments` role directory; content-addressed, so file and message share ONE R2 object. `services/ingest/src/sidestep.ts`. **Outbound**: an over-cap `Email/set create` no longer refuses `tooLarge` — every non-inline attachment becomes a FileNode under "Sent attachments" plus an expiring share link (default 30 days, minted through the real `/api/share` door), and the body gains a block stating name, size, link, and expiry date plainly. Fires ONLY when the create would otherwise refuse; under-cap mail is byte-identical. `services/jmap/src/methods/outboundSidestep.ts`. Follow-up: no thumbnails in v1 (Workers cannot resize images); the agent MCP bridge has no request origin, so agent-composed over-cap drafts still refuse `tooLarge`. |
