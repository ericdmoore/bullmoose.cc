...
### T1 — Conformance vectors, generated from TypeScript · _before any Go_
...
- `conformance/scopes.json` — every `(granted[], required) → bool` pair over the full
  ...
  `import type` from a workers-typed package. Generating this vector _replaces_ that hack
...
### T2 — The front door that delegates everything · _the seam_ — ✅ **DONE**
...
### T3 — Run the contract suite against both, in CI · _the ratchet_ — ✅ **DONE**
...
- `✅cli/006` — `watch --exec` shell injection. Note the lesson is _"a blocklist was the
  wrong shape"_, so a test that only checks the old blocklist's entries misses the point:
...
### T5 — Port the I/O contract · _the foundation everything else sits on_ — ✅ **DONE**
...
### T7 — Release · _the point of the exercise_
...
1. **`cli-go/` in this repo, or its own?** _Recommendation: this repo._ The conformance
   vectors are generated by the TS side, and a split repo turns "regenerate and both suites
   check it" into a release dance.
2. ~~Pure-Go SQLite or cgo?~~ **DECIDED: pure Go (`modernc.org/sqlite`).** cgo forfeits
   the effortless cross-compile, which is most of the reason for doing this at all.
   Revisit only if a measurement says so.
3. **Markdown rendering** currently uses `marked`. _Recommendation: the narrowest Go
   renderer that passes the existing help/format tests — or drop rich rendering in the CLI
   and keep it a webmail concern._
4. **Does the Go binary keep the same local SQLite file, or its own?**
   _Recommendation: same file, same schema._ A user mid-migration should not lose their
