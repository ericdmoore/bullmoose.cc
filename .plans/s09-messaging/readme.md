...
>
> **Trigger:** a second human actually needs to talk to you _inside_ bullmoose. Today the
...
> federation analysis below is kept because it is correct and because it explains _why_ the
...
reachable by _our own web client and little else_: the full protocol tax (stream
...
the idioms transfer directly — `/changes` with a cursor _is_ multi-device sync, state
strings _are_ consistency, the AccountDO changelog _is_ ordered delivery. The alternative
...
XMPP assumes. An agent asking _"which of these three dates works?"_ is neither an
...
> Everything below answers the original, wider question: _could a federated XMPP service run
> on this runtime?_ It is correct and worth keeping — it is the clearest statement of what
...
> proxies _to an origin_, so something still has to hold 5269. It moves which machine
...
at — a remote homeserver federating with us is _literally making HTTP requests_, which a
...
- Asking Emily-the-model for her stats gets a _narration_ — confabulatable, costly (two
  invocations + model calls), weak provenance. Querying the store gets a _fact_. This is
...
- The mail path is also doubly gated by design: the asker's governing book must contain
  ...
  for _coordination_, pure overhead for _data_.
