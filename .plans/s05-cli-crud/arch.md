...
`send` already reads its body from stdin and states the rule — _"explicit flags beat
implicit stdin"_ (`main.ts:526`) **[live]**. Generalize it:
...
**Settled, and built** (`docs/devPlan-handoff.md:67`): _"store master + recurrence rule;
**expand on demand** within a bounded window (cap the pre-compute)"_ → **on-demand,
...
`agenda` shows expanded _occurrences_ while a write targets the **master**. The data
...
| bare edit against an _occurrence_ id                   | **refuse**, with a message naming the two explicit forms |
...
**Minting is not authorizing** (`bureau.md` §5.1): who may _use_ a credential is a
