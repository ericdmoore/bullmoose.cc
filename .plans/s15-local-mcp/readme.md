...
> server-side surface no local client can reach. `s02` is building the _remote_ front
...
MCP server's job is to _spend_ that token, not to re-acquire one. The auth model is
...
So the answer to _"is the CLI the bridge for MCP auth?"_ is: **it is the bridge that makes
...
stdio MCP server that only answers tool calls is a _poller_. One that holds the
...
- **This is the fleet-host shape again** (s11 T8): one local process, one login, a
  scoped grant, N things served. The daemon serves _invocations_; this serves _tools_.
...
2. **A session may narrow, never widen.** An MCP session must not inherit `admin` merely
   ...
   software running as you; the blast radius should be the _task_, not the _account_.
...
1. **Does `mcp serve` share a process with `agent serve`?** Both are "a local daemon
   holding a token and a socket". _Recommendation: separate commands, shared internals,
   and let them merge only if running both proves annoying._
2. **Session scoping ergonomics** — a flag (`--scopes`), a prompt, or a per-client profile?
   _Recommendation: default-narrow + `--scopes`, since the client's config file is where a
   scope decision would otherwise get silently persisted._
3. **Tool-definition sharing across languages.** The definitions live in TypeScript; a Go
   CLI serving them needs the same shapes. _Recommendation: generate them into the
   conformance contract (the pattern already exists for scopes/exit codes) rather than
   hand-mirroring — this is exactly the drift the contract suite was built to catch._
4. **Which binary serves it** — Go or the Node CLI? _Recommendation: Go, once `watch` is
   native (in flight), because the whole point is one static binary with a socket._
