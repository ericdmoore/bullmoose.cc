...
> thing a rule protects against**: the gate is literally called _CJ-cannot-self-approve_.
...
_which colleague_ should handle it, and hands off. That capability is
`agents:invoke` — and `agent-integration.md` §4 defers it with a precise condition:

> _"`agents:invoke` (agent→agent pipelines) is deferred: if ever allowed, it needs a
> **chain-depth cap** and a **shared budget**."_
...
| —               | **monotonic attenuation**: a sub-task's tools, credentials and budget are a _subset_ of its parent's |
...
2. **Un-defer `agents:invoke`** — safe _because_ of (1): a delegation is just a child node
...
a confused deputy _for the whole household_. Which is why the attenuation invariant and the
