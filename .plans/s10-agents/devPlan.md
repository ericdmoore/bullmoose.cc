...
>   governing book usually lives on the agent's _own_ account where it is the owner, so
...
>   re-check the governing book (mitigated: the agent worker refuses to _propose_ to
...
> store is a lie the moment it renders — and for a _social_ agent that lie is a
...
### T1 — `allowedRecipients` as a **governed contact book** · _the bound, and the one chokepoint_
...
- **One chokepoint, in the store — not per-protocol.** A book governed by approval but writable
  ...
  `PUT`/`DELETE`, the CLI, and MCP `contacts_create_card`; four checks means the _fifth_
...
- **The mark is a per-book `writePolicy`, not a boolean** — three levels, one chokepoint:
  - `open` — direct writes under ordinary grants (an agent's own working books; today's
    behavior).
  - `propose` — _agent_ writes flow through the queue as `create-contact` proposals (the kind
  ...
  - **Governed is viral through reference.** If a governing book (or its allowlist) references
    a group in a human book — "photos@ may email the _family_ group" — that group is now part
...
- **Fail-closed, exact-match.** Unbound ⇒ **cannot send**, matching the Bureau's invariant 5
  ...
  ⇒ cannot send, _never_ "unrestricted". `allowedSenders` (inbound) is enforced at
...
### T2 — The provenance chain · _append-only, and the why comes from the proposal_
...
- **`via_proposal_id` carries the why.** Do _not_ add a free-text `reason` column — it fills
...
- **Backfill the same gap in `grant_lifecycle`.** It records `actor` but never _why_ — the
...
- **The book folds from the log — assert it.** For a _governing_ book, membership is fully
...
- **No compaction — the cancelled pairs are the crown jewels.** The tempting compaction is
  ...
  is the _record of a window in which the agent could send_ — the widen→send→narrow attack's
...
**Done when:** every add _and_ remove on a governing book appends a row; the row links the
...
### T3 — Widening is a `grant-request` proposal · _the agent asks; it does not take_
...
whose comment already describes this pattern: _"the decision is recorded here; no local write"_
...
- The agent proposes _"let me email X, because Y"_; a human approves in `/approvals`. Rationale
...
- **`needsInfo` is the third option, and grant-request is where it earns its keep**
  (`decline-taxonomy.md`): a widening met with _"why do you need X?"_ instead of a fatigued
  ...
  privilege survives contact with a busy queue _because_ this verb exists.
- **CJ may hold bounded approval authority** (the orchestration idea: granting a privilege =
  ...
  console must render it as _"CJ can widen photos@'s reach"_ — not as "CJ has contacts write."
...
### T4 — `bullmoose agents` · _the CLI config surface_ — Go-native
...
### T5 — `/agents/<id>` config panel · _the WebUI config surface_
...
- The per-agent page gains a **config panel** beside the existing activity/permissions view
  (`perAgent.ts`). Two panels, labelled _what it is_ vs _what it's doing_ — not two pages.
...
console reads `/console/*`, four routes that are _requested, not served_ (s03.E rough edge).
...
### T6 — The agent score · _depends on s07 T5, flagged not owned_
...
### T7 — Proposals must reach the human they wait on · _found live, 2026-08-13_
...
principal's data. The _composition_ is what fails.
...
single-account approvals queue can _never_ show a human their agents' work: the queue is
...
- **`/approvals` must query every reachable account**, not just the logged-in one — owned
  _and_ granted. The Go CLI's `approvals` has the same defect.
...
1. **Does `create` mint an identity, or only a binding?** A real `analyst@` needs an address;
   a lightweight in-account agent may not. _Recommendation: `--kind` decides — `analyst`/`photos`
...
2. **Do governing books hold domains, or only addresses?** `photos@`'s invitees are individual
   ...
   and `*.host` is not a contact. _Recommendation: addresses only in the book (it is a real
   ...
   allowlist — not a fake contact. Do not build it until an agent needs it._
3. **Does `remove --destroy` cascade to proposals, or orphan-and-tombstone?** _Recommendation:
   tombstone the binding, keep the proposals (they are audit), and render them under a "removed
   agent" heading rather than deleting history — the same stance grant revocation takes._
4. **Does a human operator editing the book directly also file a proposal (T3), or write
   through?** Write-through is more convenient and puts a hole in the chain. _Recommendation:
   ...
   never write through — that is the whole bound._
5. **A human-originated request ("add Bob") — does the agent's resulting write still queue?**
   ...
   he just asked for is friction. But "the human asked" arriving _by email_ is the classic
   injection vector — a message that merely looks like the owner. _Recommendation: still file
   ...
   auto-approve is off, full stop._
...
- **Serving `/console/*`** — a server task that unblocks the _existing_ console; flagged in T3,
  not owned here.
