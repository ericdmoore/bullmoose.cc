...
|---|---|
| **Kind** | projection |
| **Effort** | **E4** — only because the WebUI stack does not exist. See *Why these grades*. |
...
One hit across both slices, and it is the word *contact* in the sense of "touches the
...
Contacts and Calendars appear all over the *rest* of the arc — as live JMAP capabilities
...
**E4 — and this is the whole point of the unit.** The capability is *complete*: full CRUD
...
exactly one reason: `readme.md:73` puts *"anything on a stack that does not exist yet"* at
...
- *Human-verifiable* — a person opens a browser, creates a contact, and sees the same
...
- *Unlocks* — `s03.D`'s approval queue proposes `"create-event"` and `"create-contact"`
  ...
  writes — flagged in *Open questions*.
...
- **Batch.** `s03.C`'s invariant 5 (`arch.md:85`) — a view open is one round trip. JMAP
  ...
  `#key` / `resultOf` per RFC 8620 §3.7, so *query then get those ids* is one POST.
...
dates (`_context.md` §5). A calendar UI is where a human first *notices* that. Unit `003`
...
1. **The `I3` "unlocks" leg rests on an inference.** `s03.D` lists `"create-contact"` and
   `"create-event"` as approval-queue kinds (`arch.md:22-23`) but never writes *"and
   therefore we need contact/calendar views"*. If you read `s03.D` as only ever rendering
...
2. **Should this be folded into `s03.C` instead?** The counter-argument is real: four
   ...
   the split is what made the hole *visible*. If `s03.C` ever adopts these surfaces, this
...
4. **A calendar UI is not a small UI.** Month grid, drag-to-reschedule, timezone display,
   all-day vs timed, and recurrence *editing* (this-event / this-and-future / all) are a
...
5. **Nothing here was run.** All claims read from source. In particular I have not
   ...
   *Done when* rests on `dav.ts` handlers alone, the same caveat `_context.md` §7 states.
