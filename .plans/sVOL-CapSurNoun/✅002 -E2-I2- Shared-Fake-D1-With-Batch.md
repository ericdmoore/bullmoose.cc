...
> _§ Outcome_ at the bottom for the decision record and the answers to the Open
> Questions.
>
...
- _Unlocks_ — five ledger units name it. `013:146-148` states the edge explicitly: `Mailstore`
  calendar writes use `.batch()` and the existing fake does not implement it.
- _Not human-verifiable_ — a non-engineer cannot observe a test fake by any interface. This is
...
1. **It routes by SQL substring match.** Three literal fragments, then a catch-all at `:35` that
   returns whatever the fixture put in `rows.tool` for _any unmatched query_. That is precisely
...
3. **It is not type-checked and cannot be.** `mcp.test.ts:89-90` carries the comment _"Test file
   is excluded from tsc; the env only needs DB for this handler"_ and casts through
...
> _Fast unit tests, no workerd/miniflare: per `.plans/devPrinciples.md` the core logic is pure
...
_Cheapest by far — perhaps an afternoon._ But the routing table grows one fragment per query
...
- `D1Database` is a `declare abstract class` with no private members, so a plain object
  ...
  `run`, `all`, `raw`.
- `.plans/devPrinciples.md` explicitly authorizes this: _"leverage 3rd party fake-clients &
  fake-db-mocks, and harnesses for testing - make them as needed"_ and _"By injecting
  fake-clients most of the shell code can get coverage too without the need for real network
  access."_ This unit is that line being cashed.
...
grant_audit`, which is better. Migrate those assertions rather than preserving the array
  idiom for its own sake — but keep the array available, because asserting on the _exact SQL
  text_ is occasionally the point.
...
1. **"Unlocks every write-path test" is overstated, including in this file's own dependency
   ...
   So `013`'s done-when #2 — _"the write appears in `CalendarEvent/changes`"_ — is not testable
...
2. **`E2` is arguably wrong if the fake becomes a package.** `readme.md`'s `E4` anchor is _"New
   workspace, service, or protocol surface"_ — and `packages/test-fakes` is literally a new
...
### The fork: (c), and (b) is now a _cheaper_ future step, not a foregone one
...
(a) cannot satisfy done-when #3 or #4 _at all_: `run()` writes to an array `all()` never reads,
...
1. **It is not on the critical path.** (b) tests the _shell_: bindings, DO placement, real D1
   wire semantics. The units blocked on `002` (`004`, `006`, `007`, `013`, `014`) are blocked
   on the _write choreography_ — mutate → bump ctag → commit changelog — which is method-layer
...
now _cheaper_ to adopt, because the fixtures are real SQL against the real schema rather than
...
4. **Wrong-table bugs fail.** The old catch-all answered _any_ unmatched query from a fixture.
...
2. **`.run()` on a read.** `node:sqlite`'s `run()` reports a _stale_ `sqlite3_changes` value, so
...
3. **Rollback masking the real error.** `ON CONFLICT ROLLBACK` unwinds the transaction itself,
   so the fake's own `ROLLBACK` threw _"no transaction is active"_ and replaced the constraint
   violation with it.
4. **The DO stub dropped headers.** `services/jmap/src/index.ts:96` passes a `Request` as the
   _init_ argument and `AccountDO.upgradeWebSocket` reads `Upgrade`, so header loss produces a
...
4. **The miniflare deferral is defended above, on different grounds than this file guessed.**
   The pool _can_ be scoped per-project; that argument is withdrawn.
...
- **`services/agent/src/index.ts:329`** (`finish`) writes terminal invocation state with raw SQL
  ...
  `_context.md` footnote 13 already flags it as live in the tree. It is now _testable_ — the
