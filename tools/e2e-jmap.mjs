...
assert(
  got.bodyValues.t.value.trim() === "Hello from the draft body",
  `body round-trip: ${JSON.stringify(got.bodyValues)}`,
);
...
assert(
  sent2.totalEmails === 1 && drafts2.totalEmails === 0,
  `counts: sent=${sent2.totalEmails} drafts=${drafts2.totalEmails}`,
);
...
const up = await (
  await fetch(`${BASE}/api/upload/${ACCT}`, {
    method: "POST",
...
const down = await fetch(`${BASE}/api/download/${ACCT}/${up.blobId}/x?type=text/plain`, {
  headers: H,
});
