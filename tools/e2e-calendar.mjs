...
assert(
  CAP in session.capabilities && CAP in session.accounts[ACCT].accountCapabilities,
...
// 3. second calendar
const [[, calSet]] = await jmap([
  [
    "Calendar/set",
    {
      accountId: ACCT,
      create: {
        w: { name: "Work", color: "#3366ff" },
      },
    },
    "c1",
  ],
...
await q(
  ...
  [standupId],
  "window hits a standup occurrence",
);
await q(
  { after: "2027-12-01T00:00:00Z", before: "2027-12-31T00:00:00Z" },
  [bdayId],
...
const [[, occ]] = await jmap([
  [
    ...
    { accountId: ACCT, after: "2026-10-01T00:00:00Z", before: "2026-12-15T00:00:00Z" },
    "c4",
  ],
]);
...
assert(
  standups.every((o) => o.start.endsWith("T09:00:00")),
...
const [[, moveRes]] = await jmap([
  [
    "CalendarEvent/set",
    {
      accountId: ACCT,
      update: {
        [dentistId]: { calendarIds: { [workCal]: true } },
      },
    },
    "c8",
  ],
