...
const root = await dav(ERIC, "PROPFIND", "/dav/", {
  headers: { Depth: "0" },
...
const home = await dav(ERIC, "PROPFIND", homeHref, {
  headers: { Depth: "1" },
...
const bookList = await dav(ERIC, "PROPFIND", bookHref, {
  headers: { Depth: "1" },
...
const adaEtag = extract(
  bookList.text,
  new RegExp(`${adaId}\\.vcf</D:href><D:propstat><D:prop><D:getetag>([^<]+)</D:getetag>`),
);
assert(adaEtag?.startsWith("&quot;"), `etag rendered: ${adaEtag}`);

// 5. initial sync-collection
const sync0 = await dav(ERIC, "REPORT", bookHref, {
  headers: { Depth: "1" },
...
const mg = await dav(ERIC, "REPORT", bookHref, {
  headers: { Depth: "1" },
...
const putVcf = [
  ...
  "FN:Charles Babbage",
  "N:Babbage;Charles;;;",
  "EMAIL;type=INTERNET;type=WORK:charles@dav.test",
...
const [[, q1]] = await jmap(ERIC, [
  ["ContactCard/query", { accountId: ERIC.acct, filter: { email: "charles@dav.test" } }, "q"],
]);
assert(q1.ids.length === 1, "DAV-created card visible over JMAP");

// 9. ctag bumped by the PUT
const home2 = await dav(ERIC, "PROPFIND", bookHref, {
  headers: { Depth: "0" },
...
assert(
  sync1.text.includes("ABC-123-APPLE.vcf") && !sync1.text.includes(`${adaId}.vcf`),
...
assert(
  sync2.text.includes("ABC-123-APPLE.vcf") && sync2.text.includes("404"),
...
const [[, tmp]] = await jmap(ERIC, [
  [
    "ContactCard/set",
    {
      accountId: ERIC.acct,
      create: {
        t: { uid: "dav-e2e-temp", name: { full: "Temp Person" } },
      },
    },
    "t",
  ],
...
const [[, fam]] = await jmap(ERIC, [
  ["AddressBook/set", { accountId: ERIC.acct, create: { f: { name: "DAV Family" } } }, "f"],
]);
...
const carolHome = await dav(CAROL, "PROPFIND", homeHref, {
  headers: { Depth: "1" },
...
assert(
  carolHome.text.includes("DAV Family") && !carolHome.text.includes(bookHref),
