  ...
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
      ...
      <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
        <button
          type="button"
          class="block rounded-md bg-indigo-600 px-3 py-1.5 text-center text-sm/6 font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600 dark:bg-indigo-500 dark:hover:bg-indigo-400 dark:focus-visible:outline-indigo-500"
        >
          Add user
        </button>
      </div>
            ...
            >
                  ...
                  <th scope="col" class="relative px-7 sm:w-12 sm:px-6">
                    <div class="group absolute top-1/2 left-4 -mt-2 grid size-4 grid-cols-1">
                      <input
                        ...
                        @change="
                          selectedPeople = $event.target.checked ? people.map((p) => p.email) : []
                        "
                      />
                      <svg
                        class="pointer-events-none col-start-1 row-start-1 size-3.5 self-center justify-self-center stroke-white group-has-disabled:stroke-gray-950/25 dark:group-has-disabled:stroke-gray-50/25"
                        viewBox="0 0 14 14"
                        fill="none"
                      >
                        <path
              ...
              >
                <tr
                  ...
                  :key="person.email"
                  class="group has-checked:bg-gray-50 dark:has-checked:bg-gray-800/50"
                >
                  <td class="relative px-7 sm:w-12 sm:px-6">
                    <div
                      className="hidden group-has-checked:block absolute inset-y-0 left-0 w-0.5 bg-indigo-600 dark:bg-indigo-500"
                    />
                  ...
                  <td
                    class="py-4 pr-3 text-sm font-medium whitespace-nowrap text-gray-900 group-has-checked:text-indigo-600 dark:text-white dark:group-has-checked:text-indigo-400"
                  >
                    {{ person.name }}
                  </td>
...
const indeterminate = computed(
  () => selectedPeople.value.length > 0 && selectedPeople.value.length < people.length,
);
</script>
