  ...
  <TransitionRoot :show="open" as="template" @after-leave="query = ''" appear>
    <Dialog class="relative z-10" @close="open = false">
      ...
      <div class="fixed inset-0 z-10 w-screen overflow-y-auto p-4 sm:p-6 md:p-20">
        ...
        >
          <DialogPanel
            class="mx-auto max-w-xl transform overflow-hidden rounded-xl bg-white shadow-2xl outline-1 outline-black/5 transition-all dark:bg-gray-900 dark:-outline-offset-1 dark:outline-white/10"
          >
            <Combobox @update:modelValue="onSelect">
              ...
              <div
                v-if="query === ''"
                class="border-t border-gray-100 px-6 py-14 text-center text-sm sm:px-14 dark:border-white/10"
              >
                <GlobeAmericasIcon
              ...
              >
                <li v-for="[category, items] in Object.entries(groups)" :key="category">
                  <h2
                    class="bg-gray-100 px-4 py-2.5 text-xs font-semibold text-gray-900 dark:bg-white/5 dark:text-white"
                  >
                    {{ category }}
                  </h2>
