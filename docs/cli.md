...
_Generated by the retired Node CLI's final `gen:docs` run and FROZEN at the Node CLI's removal (2026-08-22). The live help surface is the Go binary's own (`bullmoose help`, served from `cli-go/internal/help/artifact.txt`); this page is its markdown rendering, frozen the same day._
