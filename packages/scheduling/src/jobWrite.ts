// WHERE A DELEGATION IS WRITTEN — the twin of `nodeAuthority.ts`, which is
// where one is SPENT.
//
// This module is `services/agent/src/jobs.ts`'s two write paths, moved here
// verbatim and given a bare `AuthorityDb` instead of the agent worker's `Env`.
// The move happened for the reason every move in this package has happened:
// **a second worker can now name these operations**, and two implementations
// of "create a Job node, attenuated" is the drift that makes the invariant
// decorative.
//
//   startJobRows()    mints a Job and its root node, the root attenuated
//                     against the binding's declared ceiling.
//   expandPlanRows()  turns a planner's OUTPUT — untrusted data — into tasks,
//                     if and only if every one of them attenuates.
//
// The second caller is s20 T6's plan-approval checkpoint. When the human
// approves (or redlines) a planner's sketch, `ActionProposal/set` in
// services/jmap is the thing that creates the tasks — so the expansion has to
// run inside the JMAP worker, against the same chain ceiling, with the same
// guarded INSERT and the same all-or-nothing refusal. Sharing the function is
// what makes "the venue moves, the ledger does not" true of the ENFORCEMENT
// as well as of the rows: an approved edit meets exactly the checks a
// planner's own output meets, because it is the same code.
//
// What did NOT move: `commitChanges`. Each caller owns its own changelog
// choreography — the agent worker commits immediately, while
// `ActionProposal/set` folds the entry into the single commit that covers the
// whole decision — and a shared writer that committed on its callers' behalf
// would break the second one's one-newState guarantee.
...
export async function getJobNodeRow(db: AuthorityDb, accountId: string, id: string): Promise<JobNodeRow | null> {
...
/**
 * Create a Job and its root node. The caller commits the `AgentInvocation`
 * changelog entry for `rootId`.
 *
 * Trusted-caller API in the sense that only server code reaches it — and
 * written as if the caller were not trusted anyway, because the root goes
 * through the same attenuation as any child. A ceiling that only applies below
 * the root is not a ceiling, and s20 T6's `Goal/set` is precisely the "a
 * client can start one now" day that assumption was written for.
 */
export async function startJobRows(db: AuthorityDb, spec: JobSpec): Promise<StartJobResult> {
  const binding = await db.DB.prepare(
...
/**
 * THE PLANNER'S OUTPUT BECOMES TASKS — the chokepoint. The caller commits the
 * `AgentInvocation` entries for the ids it returns.
 *
 ...
 */
export async function expandPlanRows(db: AuthorityDb, parent: JobNodeRow, plan: unknown): Promise<ExpandResult> {
  if (!parent.job_id) {
    return {
      ok: false,
  ...
  // THE USE-TIME BOUND, and it comes FIRST. Not the parent's stored envelope
  // believed verbatim: re-delegation is a USE of the delegated authority, so
  // the ceiling a plan is checked against is recomputed here from the binding
  // down through every hop (`nodeAuthority.ts`): binding ∩ root ∩ … ∩ parent.
  // A row that claims more than its ancestors held delegates the intersection
  // rather than the claim, and a binding narrowed after this Job started bites
  // the plan being expanded right now.
  //
  ...
  const effective = await effectiveNodeCeiling(db, parent);
  if (!effective.ok) {
    return {
      ok: false,
      refusals: [
  ...
  // account when IT tries to act (`nodeAuthority.ts`); it must not be able to
  // make another node's plan unanswerable.
  ...
  const now = Date.now();
  const changes = await insertJobChildren(db, parent, job, result.children, ids, now);
  ...
  return { ok: true, created: result.children.map((c) => ({ key: c.key, id: ids.get(c.key)! })) };
}
 ...
 */
export async function joinContextRows(
  db: AuthorityDb,
  node: JobNodeRow,
