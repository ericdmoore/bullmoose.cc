...
checks it against the runtime.** `docs/cli.md` is generated and therefore always consistent *with the
spec* — and silently wrong wherever the spec is.
...
Then `admin`'s error string (item 1) should be *derived* from its subcommand switch rather than
...
- **(4)(5)(6)(7)** — pure doc edits. (5) is in `packages/cli/README.md`, the others in `docs/`.
  Consider also *accepting* `--url` as an alias for `--base` since it parses and is documented
