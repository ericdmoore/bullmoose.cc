...
Don't delete the plan — the _reasoning_ in it is worth keeping. Restructure:
...
2. **§1 "Today"** (`:11-17`) → past tense, one paragraph: _"`src/` was a Fresh/Deno stub; it was
   replaced by Astro + Preact."_ The Fresh narrative has no present-tense value.
...
Three implementation choices deliberately diverged from the plan and are _better_. Record them so
...
tag: "connection guide", blurb: "" }`, so a new playbook publishes with a raw-slug `<h1>` and an
  **empty meta description** (fed to `<meta name="description">` and `og:description` via
  ...
  path _will_ be hit. A build-time assert that every collection entry has a `GUIDES` entry is cheap.
...
- **`/demo` routing needs documenting**: `services/demo-keys` claims `bullmoose.cc/demo*` on the same
  ...
  same-hostname _path_ routing). Also there is **no `/demo` link anywhere in the site** — grep of
