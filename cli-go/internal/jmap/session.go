...
// Mail capability URNs — packages/cli/src/jmap.ts USING.
const (
	MailCap       = "urn:ietf:params:jmap:mail"
	SubmissionCap = "urn:ietf:params:jmap:submission"
)

// MailUsing is the default capability list of packages/cli/src/jmap.ts (USING),
// sent by every `read` and `send` call so the request is the same one Node makes.
