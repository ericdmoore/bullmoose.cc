...
func (r *calendarRunner) eventEdit(id string) int {
	// Single-occurrence editing (--occurrence <recurrenceId>): s05 T3's
	// Done-when, unmet until #222 — the devPlan permitted refusing a BARE
	// edit aimed at an occurrence id, and REQUIRED --occurrence to write a
	// `recurrenceOverrides` entry. It writes one now.
	//
	// The shape is JSCalendar's own (RFC 8984 §4.3.3): the master carries
	// `recurrenceOverrides: { "<recurrenceId>": { …patch } }`, and one
	// occurrence differing is a patch under its own key — never a second
	// event, and never a rewrite of the series. The server-side expander
	// already honours it (packages/calendar-core expandOccurrences: "override
	// keys not generated by the rule are ADDED"), so this is the writer for
	// a reader that has always been there.
	//
	// Sent as a nested JMAP PatchObject path — `recurrenceOverrides/<id>/<prop>`
	// — precisely so the OTHER occurrences' overrides are untouched: replacing
	// the whole `recurrenceOverrides` object would silently drop them, which
	// is the failure T3's Done-when names ("leaves the other occurrences
	// untouched").
	// The id is validated ABOVE every round trip, for the reason the old
	// refusal gave: a mistyped invocation must not half-write, and must not
	// cost a fetch either.
	occurrence := ""
	if r.a.HasOccurrence {
		key, oerr := occurrenceKey(r.a.Occurrence)
		if oerr != nil {
			return die(r.s, oerr)
		}
		occurrence = key
	...
	if r.a.HasOccurrence {
		// Series-shaped properties cannot be overridden per occurrence (RFC
		// 8984 §4.3.3: recurrence rules and the override map itself are
		// master-only). Refusing NAMES the property rather than letting the
		// server reject an opaque patch.
		for _, k := range patch.Keys() {
			switch k {
			case "recurrenceRules", "recurrenceOverrides", "excludedRecurrenceRules":
				return die(r.s, bmio.Fail(
					"--occurrence cannot change "+k+" — recurrence is a property of the SERIES; "+
						"edit the master (drop --occurrence) to change how it repeats",
					bmio.ExitUsage))
			}
		}
		nested := jsobj.New()
		for _, k := range patch.Keys() {
			raw, _ := patch.Raw(k)
			// One path per property: `recurrenceOverrides/<recurrenceId>/<prop>`.
			// The JMAP PatchObject replaces exactly that leaf, so every other
			// occurrence's override — and every other property of THIS one —
			// survives the write.
			nested.SetRaw("recurrenceOverrides/"+jmapPointerEscape(occurrence)+"/"+jmapPointerEscape(k), raw)
		}
		patch = nested
	}
