...
// TestNodeCanReadTheMirrorGoCreated lived here until the Node CLI's removal:
// it drove the NODE CLI over a Go-created mirror and diffed sqlite_master
// against the schema Node built from the canonical .sql files. With one
// runtime left there is no cross-runtime to assert; the schema drift check
// (store/schema_test.go) still pins the embedded DDL to the canonical
// data-plane.sql, and cli-go/internal/store/sql/local.sql is canonical now.
